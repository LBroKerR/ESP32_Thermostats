...
bool HeatHandler::add_device_to_HeatingCircles(unsigned index, String const MACaddress, String const name, float const temp,unsigned const id){
    if(HeatingCircles==nullptr || number_of_HeatingCircles==0 ){
        return false;
    }
    if(index>=0 && index<number_of_HeatingCircles){
        if(HeatingCircles[index].addDevices(MACaddress,name,temp,id)){
            return true;
        }
    }
    return false;
...
void HeatHandler::remove_device_from_HeatingCircles(String Mac){
    if(HeatingCircles==nullptr || number_of_HeatingCircles==0){
        return;
    }
    for(unsigned i=0; i<number_of_HeatingCircles; i++){
        if(HeatingCircles[i].removeDevice(Mac)){
            break;
        }
    }
}
