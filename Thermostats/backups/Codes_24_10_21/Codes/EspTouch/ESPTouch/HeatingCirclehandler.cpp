...
bool HeatingCircleHandler::addDevices(String const MACaddress, String const name, float temp  ,unsigned const id){
    if(devices==nullptr){
        devices=new DeviceComponents();
    }
    devices->add_Component(MACaddress,name,temp,id);
    return true;
}
...
bool HeatingCircleHandler::removeDevice(String Mac){
    if(devices==nullptr){
        delete devices;
        return false;
    }
    ...
    for(unsigned i=0; i<devices->getSize(); i++){
        if(devices->getDevices()[i].getMac()==Mac){
            devices->remove_Component(Mac);
            return true;
        }
    }
    return false;
