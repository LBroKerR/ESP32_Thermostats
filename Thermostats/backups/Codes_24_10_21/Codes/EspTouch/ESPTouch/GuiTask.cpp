...
void is_user_interacted(DataHandler* data, unsigned *switch_of_screen){
   if(Turn_ON_OFF){
    ...
    else if(/*ts.tirqTouched() || */  ts.touched()){
...
void GuiTask_main(MainTask* screens, DataHandler* data){
  ...
  while(changeTask(screens,data,ui_TmpChart_series_1, &screen_number)){ 
    lv_timer_handler();
