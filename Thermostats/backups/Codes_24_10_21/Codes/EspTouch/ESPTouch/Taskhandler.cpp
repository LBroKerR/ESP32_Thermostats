...
void Taskhandler::Guitask(void*parameters){
  ...
  if (xQueueReceive(Queue, &receivedTaskHandle, portMAX_DELAY) == pdTRUE ) {
    ...
    if(data.getHeater()->getHeatingCircleHandler()[update->getSensorLocation()].getSensor()!=nullptr){
      TaskHandle_t core0;
      xTaskCreatePinnedToCore(Taskhandler::OtherTasks, "core0", 4000, update, 5, &core0, 0);
...
void Taskhandler::OtherTasks(void*parameters){
  ...
  while(true){
    ...
    wifi.main();//device()->device()!=nullptr? kimaradt helyette size ellenőrzés volt...
    autoSave->save(&data);
...
void Taskhandler::main(){
  ...
  xTaskCreatePinnedToCore(Taskhandler::BIOSTask, "core1", 4000, &myQueue1, 5, &core1, 1);
}